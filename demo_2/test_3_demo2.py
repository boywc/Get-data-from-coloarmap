...
if __name__ == "__main__":
    ...
    name =               ["Apollo11",  "Apollo12", "Apollo16", "Apollo17", "CE5", "CE6"]
    input_Lon = np.array([23.47297,        -23.4     ,15.51,      30.7717,     -51.916,  -153.998])
    input_Lat = np.array([0.67408,         3.2     , -8.97,      20.1908,      43.058,   -41.625])

    for index, name_now in enumerate(name):
        out, draw = find_data(data, input_Lon[index], input_Lat[index], box_leftdown_real, box_rightup_real, range_box=5,filter=0.1, show=True)
        draw_board[draw[0]:draw[1], draw[2]:draw[3]] = 255

        print(name_now, "   ", out)

    cv2.imwrite("./demo_2/draw.jpg", draw_board)
